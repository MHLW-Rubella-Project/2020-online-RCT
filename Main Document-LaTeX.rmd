...
abstract: |
  ...
  \vskip\baselineskip
  \noindent
  \textit{Keywords}: Text message reminders; Free vouchers; Online field experiment; Rubella; Antibody test; Vaccination
author:
  - name: Hiroki Kato
    inst-id: a
    note: |
      Corresponding author. E-mail address: hkato.econ@r.hit-u.ac.jp Present address: Hitotsubashi Institute for Advanced Study, Hitotsubashi University, 2-1 Naka, Kunitachi, Tokyo 186-8601, Japan.
  - name: Shusaku Sasaki
    inst-id: b
    note: |
      E-mail address: ssasaki.econ@cider.osaka-u.ac.jp
  - name: Fumio Ohtake
    inst-id: b
    note: |
      E-mail address: ohtake@cider.osaka-u.ac.jp
inst:
  - id: a
    name: Graduate School of Economics, Osaka University, 1-7, Machikaneyama, Toyonaka, Osaka 560-0043, Japan
  - id: b
...
options(
  ...
  modelsummary_format_numeric_latex = "mathmode"
)
```
...
However, it has been theoretically suggested that vaccination rates may not reach the socially optimal level due to such positive externalities [@Brito1991a; @Francis1997; @Stiglitz2000]. Even if people are altruistic and gain utility from the social benefits of their vaccinations reducing the probability of others becoming infected, they still have an incentive to free-ride on others' vaccinations and not receive the vaccines themselves because their probability of infection is reduced by others' vaccinations. Empirical studies report that increased rates of others' vaccinations reduce the likelihood of people being vaccinated [@Hershey1994; @Ibuka2014].
...
The participants' observable characteristics are balanced across the experimental arms in both *default incentive* and *opt-in incentive* group (Table C2 and C3 in Supplementary Material C). Thus, we first report the difference-in-mean test (t-test) for each group.

### Difference-in-mean Test

```{r ttest-int-test, fig.cap = "Effect of the Text Message Reminders on the Intention to Undertake Antibody Testing. Notes: Numbers in the figure indicate the proportion of each experimental arm. Error bars indicate the standard error of the mean. P-values of t-tests for the difference-in-mean are in square brackets."}

main$ttest()
```

Figure \@ref(fig:ttest-int-test) shows the intention to undertake antibody testing, which is our primary outcome, in each experimental arm for the *default incentive* group (Panel A) and *opt-in incentive* group (Panel B). In the *default incentive* group, the Altruistic message increases the intention to undertake antibody testing by a statistically significant $14.3$ pp compared with the MHLW (Control) message ($35.1$% in the Altruistic message group versus $20.8$% in the MHLW (Control) message group). On the contrary , in the *opt-in incentive* group, the Altruistic message increases the intention to undertake antibody testing by $5.3$ pp, which is not statistically significant ($32.9$% in the Altruistic message group versus $27.6$% in the MHLW (Control) message group).[^exact-p-1] The effect of the Altruistic message could differ between the two groups, as the subsequent regression analysis also suggests.

[^exact-p-1]: These results are robust to an alternative statistical test (Fisher's exact test). See Table C4 in the Supplementary Material C.

```{r ttest-int-vacc, fig.cap = "Effect of the Text Message Reminders on the Intention to be Vaccinated. Notes: Numbers in the figure indicate the proportion of each experimental arm. Error bars indicate the standard error of the mean. P-values of t-tests for the difference-in-mean are in square brackets."}

main$ttest(outcome_test = FALSE, label_y_pos = 70)
```

Figure \@ref(fig:ttest-int-vacc) depicts the intention to be vaccinated, which is our secondary outcome, in each experimental arm for the *default incentive* group (Panel A) and *opt-in incentive* group (Panel B). In both incentive groups, most of the text message reminders, including the Altruistic message, do not statistically significantly increase the intention to be vaccinated compared with MHLW (Control). In the *opt-in incentive* group, the Social Comparison message may lower the intention to be vaccinated compared with the MHLW (Control) message ($44.6$% in the Social Comparison message group versus $52.8$% in the MHLW (Control) message group).[^mechanism]
...
Since a few respondents dropped out between waves 1 and 2, We again test a balance check  and find that the observable characteristics are balanced across the experimental arms in both the *default incentive* and the *opt-in incentive* groups (see Tables C5 and C6 in Supplementary Material C). Therefore, we present the difference-in-mean test (t-test) for each group first.

### Difference-in-mean Test

```{r ttest-act-test, fig.cap = "Effect of the Text Message Reminders on the Actual Uptake of Antibody Testing. Notes: Numbers in the figure indicate the proportion of each experimental arm. Error bars indicate the standard error of the mean. P-values of t-tests for the difference-in-mean are in square brackets."}
...
Figure \@ref(fig:ttest-act-test) shows the actual uptake of antibody testing, which is our primary outcome for behavior, in each experimental arm for the *default incentive* group (Panel A) and *opt-in incentive* group (Panel B). As in the intention case, the Altruistic message statistically significantly increases the actual uptake of antibody testing compared with the MHLW (Control) message by $7.4$ pp in the *default incentive* group ($10.9$% in the Altruistic message group versus $3.5$% in the MHLW (Control) message group). On the contray, in the *opt-in incentive* group, the Altrustic message does not affect the actual uptake of antibody testing.[^exact-p-2] The effect of the Altrustic message could differ as the subsequent regression analysis also suggests.

[^exact-p-2]: These results are robust to an alternative statistical test (Fisher's exact test). See Table C7 in the Supplementary Material C.

The Selfish message may boost the actual uptake of antibody testing in the *default incentive* group by $5.5$ pp (9% in the Selfish message group versus $3.5$% in the MHLW (Control) message group).[^exact-p-3] Moreover, the Social Comparison message may also increase the actual uptake of antibody testing in the *opt-in incentive* group ($4.9$% in the Social Comparison message group versus $3.5$% in the MHLW (Control) message group). These effects are statistically significant at the 10% level.

[^exact-p-3]: This result is not robust to alternative statistical test (Fisher's exact test). See Table C7 in Supplementary Material C.

```{r ttest-act-vacc, fig.cap = "Effect of the Text Message Reminders on Vaccination Rates. Notes: Numbers in the figure indicate the proportion of each experimental arm. Error bars indicate the standard error of the mean. P-values of t-tests for the difference-in-mean are in square brackets."}
...
[^exact-p-4]: These results are not robust to alternative statistical test (Fisher's exact test). See Table C7 in Supplementary Material C.
...
[^about-other-message]: In addition to the Altruistic message, the Selfish and Social Comparison messages raise the intention to undertake antibody testing and actual uptake of antibody testing among men who already received the vaccination vouchers by default. However, these effect sizes are insufficiently large to maintain statistical power, thereby requiring re-examination with a larger sample size.
...
The second direction concerns the channels of the effect on vaccination rates. We find that the Altruistic and Social Comparison messages may increase vaccination rates among men who received free vaccination vouchers by default. The effect of the text message reminders on vaccination rates is created through two channels. First, the messages increase the number of negative individuals who need to be vaccinated. Second, the messages encourage unvaccinated negative individuals to be vaccinated. Our data suggest that the first channel is more important than the second. Table C11 in the Supplementary Material C shows the number of men who underwent antibody testing, who had a negative antibody test, and who were vaccinated in each experimental arm in the *default incentive* group. Most of those with negative antibody test results were vaccinated in all the experimental arms. Furthermore, the number of negative antibody tests varied between the experimental arms. In particular, 25% of the antibody tests were negative in the MHLW (Control) message group compared with 50% ($=7/14$) and 56% ($=5/9$) for the Altruistic and Social Comparison messages, respectively, which may have a positive effect on vaccination rates.[^negative-selfish] Thus, the experimental arms with higher vaccination rates have a greater negative antibody testing rate, resulting in a positive effect on vaccination.
