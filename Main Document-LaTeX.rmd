...
Figure \@ref(fig:ttest-act-test) shows the actual uptake of antibody testing in each experimental arm for the *default incentive* group (Panel A) and *opt-in incentive* group (Panel B). As in the intention case, the Altruistic message statistically significantly increases the actual uptake of antibody testing compared with the MHLW (Control) message by $7.4$ pp in the *default incentive* group ($10.9$% in the Altruistic message group versus $3.5$% in the MHLW (Control) message group).[^exact-p-act]

[^exact-p-act]: For robustness, we conduct the Fisher's exact test and obtain same results (Table C8 in Supplementary Material C).
