...
\date{Last updated on October 13, 2023}
...
Altruistic (and possibly Social Comparison) messages encourage men who received free vouchers by default to obtain antibody testing and be vaccinated. In this subsection, we attempt to evaluate the effect of these messages in the \emph{default incentive} group in terms of monetary value. That is, we estimate the extent to which the text reminders increase the monetary value of the rubella vaccination. In other words, we estimate an amount of additional subsidy equivalent to the message effects in terms of government subsidies.
...
This argument is related with the targeting of interventions \citep[e.g.,][]{Finkelstein2019}. That is, the question is whether marginal individuals whose behavior is changed by intervention receive significant benefits from a policy. In our context, we suggest that the Altruistic message increases the number of negative individuals who receive a large benefit (vaccination) from the FY2019 vaccination campaign. However, since the difference in the ratio of negative antibody tests is not statistically significant, the experimental data in this study alone do not allow us to fully explore this argument, and this is a topic for future research.
